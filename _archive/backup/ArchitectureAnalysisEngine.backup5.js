...
export class ArchitectureAnalysisEngine {
    ...
    async analyze(root) {
        ...
        const sourceFiles = await Promise.all(fileNodes.map(node => this.sourceLoader.load(node.path)));
        const model = this.parser.parse(sourceFiles);
        const graph = this.graphBuilder.build(model);
        const violations = await this.ruleEngine.validate(graph);
